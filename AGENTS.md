...
- `src/shuffling.ts` — Shuffle service tag + implementations (random, identity,
  reverse)
...
- `docs/adr-001-react-effect.md` — proposed architecture for Effect-React glue
  layer
...
- Reusable service providers as test constants (e.g. `identityShuffle`,
  `reverseShuffle`)
- Engine must stay UI-independent — no rendering, no I/O, no React
- Single-file modules until they feel cramped
- Show code proposals without editing files; the user edits generated code
  themselves
